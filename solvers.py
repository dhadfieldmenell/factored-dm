...
class ParticleRetALPv3(ParticleRetALP):

    """
    Same as above but leverages seperability wrt the particles (with a dual-gradient/admm method)

      min          rho   + sum(V) + C * w.dot(w) + D * sum(abs(xi))
 w, w_i, V, b, b_i, rho, rho_n, xi

      s.t.        V[t, n] >= rho_i(n)
                  V[t, n] >= mu[n] + gamma * V[t+1, n]
                  w_i(n).dot(phi[t, n]) + b_i(n)[t] = mu[n] + gamma * V[t+1, n] + pos_xi[t, n] - neg_xi[t, n]
    ...
    def __init__(self, gamma, feats, T, N,
                 C=1, D=100, verbose=False, batchsize=5):        
        self.model = grb.Model()   
        self.model.setParam('OutputFlag', False)
        self.model.setParam('BarConvTol', 1e-2)
        self.init_solve=True
        ...
        for t in range(self.V.shape[0]):
            for n in range(self.V.shape[1]):
                if self.gamma < 1:
                    ...
                    # use finite horizon
                    # rho is replaced with rho/T
                    self.model.addConstr(self.V[t, n] >= (self.rho / T) * (T-t-1) )
        self.model.update()
        # so we can update them
        self.particle_constrs = dict([(n, []) for n in range(self.N)])
...
def _particle_ret_val(rho, lmbda, gamma, (R, mu, phi), V, clf):
    """
    R: set of reward sequences R[i, t] is the reward from sequence i at time j

    phi: features of observation histories phi[t, i, :] is vector of
    features of sequence i from times 0:t
    """
    N, T = R.shape    
    Y = V > rho[None, :]
    weights = np.abs(mu - lmbda)

    tau = np.zeros(V.shape, dtype=int)
    tau[:, 0] = 1
    alive = np.ones(N, dtype=np.int)
    # ret_costs = np.ones(N)
    max_T = 0
    for t in range(1, T):
        alive = np.nonzero(tau[:, t-1])[0]
        # if we have retired all bad samples
        if len(np.unique(Y[alive, t])) <=  1:
            break
        clf.fit(phi[t, alive, :], Y[alive, t], weights[alive])
        tau[alive, t] = clf.predict(phi[t, alive, :])
        # ret_costs[tau[:, t]] += np.power(gamma, t)
        max_T = t
    # do the value iteration backup
    for t in reversed(xrange(max_T + 1)):
        V[:, t] = R[:, t] + gamma*(
            tau[:, t+1]*V[:, t+1] + (1-tau[:, t+1]) * rho[t+1])
    
    return np.maximum(np.mean(V[:, 0]), rho[0])

def wrap_particle_ret_val(gamma, (R, mu, phi), clf):
    N, T = R.shape
    V = np.ones((N, T+1)) * mu[:, None] / (1-gamma)
    return lambda r: _particle_ret_val(np.ones(T+1) * r, r*(1-gamma),
                                       gamma, (R, mu, phi), V[:], clf)

def wrap_fh_particle_ret_val(gamma, (R, mu, phi), clf, H):
    N, T = R.shape
    assert T < H
    R = R[:, :T]
    # rate of rewards is (1 - gamma) * rho / (1 - gamma^H)
    def fh_coef(t):
        if gamma == 1:
            return H - t
        return (1 - np.power(gamma, H - t)) / (1 - gamma)


    fh_coefs = np.array([fh_coef(i) for i in range(T+1)])
    V = np.ones((N, T+1))
    V *= mu[:, None]
    V *= fh_coefs[None, :]
    return lambda r: _particle_ret_val(fh_coefs * (r / fh_coefs[0]),
                                       r / fh_coefs[0], gamma, 
                                       (R, mu, phi), V[:], clf)
